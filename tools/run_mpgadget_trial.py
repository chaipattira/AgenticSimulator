...
On failure (out-of-prior params/resolution, or any SLURM stage failing): prints an ERROR
...
from simulator.mpgadget_wrapper import MPGadgetJobError, MPGadgetSimulator, OutOfPriorError
...
def main():
    ...
    sim = MPGadgetSimulator(shenqi_root=default_shenqi_root(_ROOT), csv_path=cwd / "runs.csv",
                            prior_bounds=cfg)
    ...
    with open(cwd / "runs.csv", "a", newline="") as f:
